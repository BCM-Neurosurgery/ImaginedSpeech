...
%LOAD_BLOCK2_CONTENT Validate schedule and preload all unique image pixels.

if ~isfile(contentPath)
    error('ImaginedSpeech:MissingBlock2Content', 'Block 2 manifest not found: %s', contentPath);
end
...
for index = 1:numel(required)
    if ~isfield(block, required{index})
        error('ImaginedSpeech:InvalidBlock2Content', 'Missing field: %s', required{index});
    end
end
if block.schema_version ~= 1
    error('ImaginedSpeech:InvalidBlock2Content', 'Unsupported Block 2 schema version.');
end
