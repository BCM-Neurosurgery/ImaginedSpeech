function block = load_block1_content(contentPath)
%LOAD_BLOCK1_CONTENT Validate schedules and preload all unique speech audio.

if ~isfile(contentPath), error('ImaginedSpeech:MissingBlock1Content','Block 1 manifest not found: %s',contentPath); end
...
for index=1:numel(required), if ~isfield(block,required{index}), error('ImaginedSpeech:InvalidBlock1Content','Missing field: %s',required{index}); end, end
if block.schema_version~=1, error('ImaginedSpeech:InvalidBlock1Content','Unsupported Block 1 schema version.'); end
...
if ~isfile(schedulePath)||~isfile(audioManifestPath), error('ImaginedSpeech:MissingBlock1Data','Schedule or audio manifest is missing.'); end
