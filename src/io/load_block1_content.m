...
%LOAD_BLOCK1_CONTENT Validate and preload all Block 1 content and audio.

if ~isfile(contentPath)
    error('ImaginedSpeech:MissingBlock1Content', 'Block 1 content not found: %s', contentPath);
end
...
for index = 1:numel(required)
    if ~isfield(block, required{index})
        error('ImaginedSpeech:InvalidBlock1Content', 'Missing field: %s', required{index});
    end
end
if block.schema_version ~= 1 || numel(block.stories) ~= 2
    error('ImaginedSpeech:InvalidBlock1Content', ...
        'Block 1 requires schema version 1 and exactly two stories.');
end
...
if numel(unique(questionIds)) ~= numel(questionIds)
    error('ImaginedSpeech:InvalidBlock1Content', 'Question IDs must be unique.');
end
