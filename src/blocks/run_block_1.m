...
%RUN_BLOCK_1 Present two stories followed by comprehension questions.

content = load_block1_content(config.block1.content_file);
oldSkip = Screen('Preference', 'SkipSyncTests', double(config.display.skip_sync_tests));
state = struct('audio', [], 'eventFile', -1, ...
    'responseFile', -1);
screenCleanup = onCleanup(@() cleanupBlock1Screen(oldSkip));
...
state.eventFile = fopen(fullfile(config.session.directory, ...
    ['block1_' runId '_events.csv']), 'w');
state.responseFile = fopen(fullfile(config.session.directory, ...
    ['block1_' runId '_responses.csv']), 'w');
if state.eventFile < 0 || state.responseFile < 0
    error('ImaginedSpeech:OutputOpenFailed', 'Could not create Block 1 output files.');
end
fileCleanup = onCleanup(@() cleanupBlock1Files( ...
...
for storyIndex = 1:numel(content.stories)
    ...
    Screen('Flip', window);
    if ~waitForEnter(keys, config.block1.test_auto_advance_seconds)
        task_killed;
    end
    ...
    if ~waitForStory(state.audio, keys.escape, config.block1.test_max_story_seconds)
    ...
    for questionIndex = 1:numel(story.questions)
        ...
        Screen('Flip', window, questionOnset + 0.5 * ifi);
        KbQueueFlush;

        pickTime = NaN;
        while true
            autoDelay = config.block1.test_auto_advance_seconds;
...
Screen('TextSize', window, config.block1.title_size);
DrawFormattedText(window, titleText, 'center', RectHeight(rect) * 0.32, config.display.text_rgb(:)');
Screen('TextSize', window, config.block1.question_size);
...
for index = 1:numel(question.choices)
    ...
    if index == highlight
        prefix = '> ';
        color = config.block1.highlight_rgb(:)';
    end
    if index == selected
        prefix = 'X ';
        color = config.block1.selected_rgb(:)';
    end
    Screen('TextSize', window, config.block1.choice_size);
...
Screen('TextSize', window, config.block1.footer_size);
...
comment = sprintf('B1_%s story=%s', eventName, storyId);
